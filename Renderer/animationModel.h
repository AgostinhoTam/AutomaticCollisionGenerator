...
public:
	...
	void AddBlendRatio(const float BlendRatio){ m_BlendRatio += BlendRatio; }
	void AddCurrentAnimationFrame(const unsigned int frame) { m_CurrentFrame += frame; }
	void AddNextAnimationFrame(const unsigned int frame) { m_NextFrame += frame; }
};