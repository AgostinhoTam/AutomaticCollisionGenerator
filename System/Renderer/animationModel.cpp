...
void AnimationModel::Draw()
{
    ...
    for (unsigned int m = 0; m < m_AiScene->mNumMeshes; m++)
    {
        ...
        {
            material.Diffuse = XMFLOAT4(0.2f, 0.2f, 1.0f, 0.5f);
            material.Ambient = material.Diffuse;
            Renderer::SetBlendState(BLEND_MODE::BLEND_MODE_ALPHABLEND);
...
void AnimationModel::Load(const char* FileName, GameObject* Owner)
{
    ...
    m_AiScene = m_Importer->ReadFile(
        FileName, aiProcessPreset_TargetRealtime_MaxQuality | aiProcess_ConvertToLeftHanded);
    assert(m_AiScene);
    ...
    for (unsigned int i = 0; i < m_AiScene->mNumTextures; i++)
    {
        ...
        if (aitexture->mHeight == 0)
        {
            ...
            CreateShaderResourceView(Renderer::GetDevice(), image.GetImages(), image.GetImageCount(), metadata,
                                     &texture);
            assert(texture);
...
float AnimationModel::CalculateMAD(std::vector<float>& radiusList)
{
    const size_t size = radiusList.size();
    if (size == 0)return 0.0f;
    //  中央
    const size_t mid = size / 2;
    //　中央値計算
    std::nth_element(radiusList.begin(), radiusList.begin() + mid, radiusList.end());
    float upper = radiusList[mid];
    ...
    {
        std::nth_element(radiusList.begin(), radiusList.begin() + (mid - 1), radiusList.begin() + mid);
        float lower = radiusList[mid - 1];
        median = 0.5f * (upper + lower);
    }

    //  絶対偏差配列
    std::vector<float> madList;
    madList.reserve(size);
    for (float radius : radiusList)
    {
        madList.emplace_back(std::fabs(radius - median)); //　距離を絶対値にする
    }
    std::nth_element(madList.begin(), madList.begin() + mid, madList.end());
    float upperMAD = madList[mid];
    float mad = -FLT_MAX;
    //  奇数チェック
    if (size % 2 == 1)
    {
    ...
    {
        std::nth_element(madList.begin(), madList.begin() + (mid - 1), madList.begin() + mid);
        float lowerMAD = madList[mid - 1];
        mad = 0.5f * (upperMAD + lowerMAD);
    }

    //  ロバスト半径
    float radius = median + 1.4826f * mad; //  ロバスト推定1.4826
    
    //  madが０の時の対策
    if (mad == 0.0f)    
    {
        size_t selectThreeQuarters = (size * 3) / 4;
        std::nth_element(radiusList.begin(), radiusList.begin() + selectThreeQuarters, radiusList.end());
        radius = radiusList[selectThreeQuarters];
    }
    return radius;
}
